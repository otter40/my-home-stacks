...
- [X] `pages/recipes/recipes`：类型/菜系筛选 + 菜名搜索 + 卡片「加入今天吃」按钮
- [X] `pages/recipes/edit`：类型(多选)/菜系(多选)/提前备菜(单选) + 从食材库选食材(含主料开关)动态行 + 做法；删除二次确认
...
### 2026-06-12: 库存修正 + Phase E 菜谱页 ✅
- 修正
  - 库存编辑「状态多选」选不上：根因 WXML 不支持 `array.indexOf()` 方法调用，改为预计算 `statusOptions:[{name,on}]`（同理菜谱编辑的类型/菜系多选也用 on 标记）
  - 库存「需要购买」按钮文案改为「进货」
- 菜谱列表 `pages/recipes/recipes`：搜索 + 类型行筛选 + 菜系行筛选（两条横向 chip）；卡片显示类型/菜系 chip + 提前备菜 + 主料一览 + 「加入今天吃/✓今天吃」切换 + 编辑/删除（二次确认）+ FAB
- 菜谱编辑 `pages/recipes/edit`：菜名、类型多选 chip（可新增）、菜系多选 chip（可新增）、提前备菜 picker、食材动态行（从食材库 picker 选 / ＋新建食材、数量、单位、主料↔辅料切换，选食材自动带默认单位与按分类的主料默认）、做法
- 待验证：列表筛选/搜索；新增菜谱（多选类型菜系、加食材行、切主料）保存后回列表正确；加入今天吃联动首页；编辑/删除正常

### 2026-06-12: 状态多选二次修正 + 首页冰箱建议改摘要入口 ✅
- 状态多选再修：把 chip 由 `<text>` 改为 `<view>` 点击目标，选中加「✓」前缀 + 实底高亮（更可靠、视觉无歧义）
...
### 2026-06-12: 反馈修正一批 ✅
- 做完了扣库存前增加 `wx.showModal` 确认弹窗（首页）
- 库存状态选择器再改为最朴素的 ☐/☑ 文字行点击（彻底保证可见，弃用 checkbox 组件）
- 保质期支持手动输入（YYYY-MM-DD 输入框）+ 📅 选择 + 清除
- 库存新增时食材库为空的处理：picker 为空时显示「食材库为空，请先新建」提示（根因多为用户在「我的」点了清空数据，可用「恢复示例数据」找回）
- 提示：若食材/库存意外为空，→ 我的 → 恢复示例数据
